...
public class OperationIdInterceptor extends OncePerRequestFilter {
	...
	public void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		// filter can be called several times so taking precautions to not add headers
		// more than once to response
		if (request.getHeader(Constants.OPERATION_ID) == null) {
			if (response.getHeader(Constants.OPERATION_ID) == null) {
				if (StringUtils.isEmpty(opid)) {
					...
					LOGGER.trace(
							"response from: {} did not contain an {} header. Including one on the response: "
									+ operationId,
